...
class DatePartRelatedExpr(LogicalExpr):
    ...
    def to_column_field(self, plan: LogicalPlan, session_state: BaseSessionState) -> ColumnField:
        self.is_date = _check_date_type(self.expr, plan, session_state)
...
class MilliSecondExpr(DatePartRelatedExpr):
    ...
    def __str__(self):
        return f"MILLISECOND({self.expr})"
...
class DateConversionExpr(LogicalExpr):
    ...
    def get_default_format(cls) -> str:
        ...
        raise NotImplementedError(f"get_default_format not implemented for {type(cls)}")
    
    @classmethod
    def get_return_type(cls) -> DataType:
        """Returns the return type for the expression."""
        raise NotImplementedError(f"get_return_type not implemented for {type(cls)}")
...
class ToTimestampExpr(DateConversionExpr):
    ...
    def __str__(self):
        return f"TO_TIMESTAMP({self.expr})"
...
class NowExpr(LogicalExpr):
    ...
    def __str__(self):
        return f"NOW(as_date={self.as_date})"
...
class DateTruncExpr(LogicalExpr):
    ...
    def __init__(self, expr: LogicalExpr, unit: str):
        super().__init__()
        self.expr = expr
        self.original_unit = unit
        self.unit = self._get_polars_date_trunk_unit(self.original_unit)

    def __str__(self):
        return f"DATE_TRUNC(unit={self.unit})"

    def to_column_field(self, plan: LogicalPlan, session_state: BaseSessionState) -> ColumnField:
        self.is_date = _check_date_type(self.expr, plan, session_state)
        return ColumnField(str(self), DateType if self.is_date else TimestampType)
    ...
    @staticmethod
    def _get_polars_date_trunk_unit(unit: str) -> str:
        """Returns the Polars date trunk unit for a given unit.
        ...
        return "1" + POLARS_DURATION_UNITS_MAP.get(unit)
...
class DateAddExpr(LogicalExpr):
    ...
    def __str__(self):
        return f"DATE_ADD(days={self.days})"
...
class TimestampAddExpr(LogicalExpr):
    ...
    def __init__(self, expr: LogicalExpr, quantity: LogicalExpr, unit: str):
        self.expr = expr
        ...
        self.unit = POLARS_DURATION_UNITS_MAP.get(self.original_unit)

    def __str__(self):
        return f"TIMESTAMP_ADD(quantity={self.quantity}, unit={self.unit})"
...
class DateFormatExpr(LogicalExpr):
    ...
    def __str__(self):
        return f"DATE_FORMAT(format={self.format})"
...
class DateDiffExpr(LogicalExpr):
    ...
    def __init__(self, expr1: LogicalExpr, expr2: LogicalExpr):
        self.expr1 = expr1
        self.expr2 = expr2

    def __str__(self):
        return f"DATEDIFF(expr1={self.expr1}, expr2={self.expr2})"

    def to_column_field(self, plan: LogicalPlan, session_state: BaseSessionState) -> ColumnField:
        _check_date_type(self.expr1, plan, session_state)
        _check_date_type(self.expr2, plan, session_state)
        return ColumnField(str(self), IntegerType)
...
class TimestampDiffExpr(LogicalExpr):
    ...
    def __init__(self, expr1: LogicalExpr, expr2: LogicalExpr, unit: str):
        self.expr1 = expr1
        self.expr2 = expr2
        self.unit = unit

    def __str__(self):
        return f"TIMESTAMP_DIFF(expr1={self.expr1}, expr2={self.expr2}, unit={self.unit})"
    ...
    def _eq_specific(self, other: TimestampDiffExpr) -> bool:
        return self.expr1 == other.expr1 and self.expr2 == other.expr2 and self.unit == other.unit


# UTILITY FUNCTIONS


def _check_date_type(expr: LogicalExpr, plan: LogicalPlan, session_state: BaseSessionState) -> bool:
    """Checks if the expression is a date or timestamp type.

    Returns True if the expression is a date type, False if it is a timestamp type.
    Raises an error if the expression is not a date or timestamp type.
    """
    col_field = expr.to_column_field(plan, session_state)
    if col_field.data_type in [DateType, TimestampType]:
        if col_field.data_type == DateType:
            return True
        else:
            return False
    else:
