...
def year(column: ColumnOrName) -> Column:
    ...
    Example:
        ...
        df.select(year(col("date"))).to_dicts()
...
def month(column: ColumnOrName) -> Column:
    ...
    Example:
        ```python
        # dates: "2025-01-01", "2025-01-02", "2025-01-03"]
        df.select(month(col("date"))).to_dicts()
        # Output: [{'month': 1}, {'month': 1}, {'month': 1}]
        ```
    """
...
def day(column: ColumnOrName) -> Column:
    ...
    Example:
        ```python
        # dates: "2025-01-01", "2025-01-02", "2025-01-03"]
        df.select(day(col("date"))).to_dicts()
...
def hour(column: ColumnOrName) -> Column:
    ...
    Example:
        ...
        df.select(hour(col("ts"))).to_dicts()
...
def minute(column: ColumnOrName) -> Column:
    ...
    Example:
        ...
        df.select(minute(col("ts"))).to_dicts()
...
def second(column: ColumnOrName) -> Column:
    ...
    Example:
        ...
        df.select(second(col("ts"))).to_dicts()
...
def millisecond(column: ColumnOrName) -> Column:
    ...
    Example:
        ...
        df.select(millisecond(col("ts"))).to_dicts()
...
def to_date(column: ColumnOrName, format: Optional[str] = None) -> Column:
    ...
    Example:
        ...
        df.select(to_date(col("date_str"), format="MM-dd-yyyy")).to_dicts()
...
def to_timestamp(column: ColumnOrName, format: Optional[str] = None) -> Column:
    ...
    Example:
        ...
        df.select(to_date(col("date_str"), format="MM-dd-yyyy HH:mm:ss")).to_dicts()
...
def current_timestamp() -> Column:
    ...
    Example:
        ```python
        df.select(current_timestamp()).to_dicts()
        # Output: [{'date': '<current date and time>'}]
        ```
    """
    return Column._from_logical_expr(NowExpr())
...
def current_date() -> Column:
    ...
    Example:
        ```python
        df.select(current_date()).to_dicts()
...
@validate_call(config=ConfigDict(strict=True, arbitrary_types_allowed=True))
def date_trunc(column: ColumnOrName, unit: str) -> Column:
    ...
    Example:
        ...
        df.select(date_trunc(col("date"), "year")).to_dicts()
        # Output: [{'date': '2025-01-01'}, {'date': '2025-01-01'}, {'date': '2025-01-01'}]
        ```
    """
    if unit not in ["year", "month", "day", "hour", "minute", "second", "millisecond"]:
        raise ValueError(f"Unsupported unit: {unit}. Supported units are: year, month, day, hour, minute, second, millisecond.")

    return Column._from_logical_expr(DateTruncExpr(Column._from_col_or_name(column)._logical_expr, unit.lower()))
...
def date_add(column: ColumnOrName, days: Union[int, ColumnOrName]) -> Column:
    ...
    Example:
        ```python
        # dates: "2025-01-01", "2025-02-01", "2025-03-01"]
        df.select(date_add(col("date"), 1)).to_dicts()
...
def date_sub(column: ColumnOrName, days: Union[int, ColumnOrName]) -> Column:
    ...
    Example:
        ```python
        # dates: "2025-01-01", "2025-02-01", "2025-03-01"]
        df.select(date_sub(col("date"), 1)).to_dicts()
...
def timestamp_add(column: ColumnOrName, quantity: Union[int, ColumnOrName], unit: str) -> Column:
    ...
    Example:
        ...
        df.select(timestamp_add(col("ts"), 1, "day")).to_dicts()
    ...
    return Column._from_logical_expr(TimestampAddExpr(Column._from_col_or_name(column)._logical_expr, quantity_expr, unit.lower()))
...
def date_format(column: ColumnOrName, format: str) -> Column:
    ...
    Example:
        ...
        df.select(date_format(col("date"), "MM-dd-yyyy hh:mm:ss a")).to_dicts()
...
def datediff(column1: ColumnOrName, column2: ColumnOrName) -> Column:
    """Calculates the number of days between two date/timestamp columns.

    Args:
        column1: The first column to calculate the difference from.
        column2: The second column to calculate the difference from.
    ...
    Example:
        ```python
        # date1: "2025-01-01", "2025-02-01", "2025-03-01"]
        # date2: "2025-01-02", "2025-02-02", "2025-03-02"]
        df.select(datediff(col("date1"), col("date2"))).to_dicts()
        # Output: [{'date_diff': 1}, {'date_diff': 1}, {'date_diff': 1}]
        ```
    """
    return Column._from_logical_expr(
        DateDiffExpr(
            Column._from_col_or_name(column1)._logical_expr,
            Column._from_col_or_name(column2)._logical_expr))

def timestamp_diff(column1: ColumnOrName, column2: ColumnOrName, unit: str) -> Column:
    ...
    Example:
        ```python
        # ts1: "2025-01-01 10:00:00", "2025-02-01 11:00:00", "2025-03-01 12:00:00"]
        # ts2: "2025-01-02 10:00:00", "2025-02-02 11:00:00", "2025-03-02 12:00:00"]
        df.select(timestamp_diff(col("ts1"), col("ts2"), "day")).to_dicts()
        # Output: [{'ts_diff': 1}, {'ts_diff': 1}, {'ts_diff': 1}]
        ```
    """
    if unit not in ["year", "month", "day", "hour", "minute", "second", "millisecond"]:
        raise ValueError(f"Unsupported unit: {unit}. Supported units are: year, month, day, hour, minute, second, millisecond.")
