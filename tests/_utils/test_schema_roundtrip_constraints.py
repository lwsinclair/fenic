...
def test_roundtrip_primitives_optional_literal():
    G = _roundtrip(PrimitivesAndOptional)
...
def test_roundtrip_string_constraints():
    G = _roundtrip(StringConstraints)
    assert G.model_validate({"s": "abc"}).s == "abc"
    with pytest.raises(ValidationError):
        G.model_validate({"s": "ab"})  # too short
    with pytest.raises(ValidationError):
        G.model_validate({"s": "abcdef"})  # too long
    with pytest.raises(ValidationError):
        G.model_validate({"s": "zzzzz"})  # pattern mismatch


def test_roundtrip_number_constraints():
    G = _roundtrip(NumberConstraints)
    assert G.model_validate({"i": 2, "f": 1.0}).i == 2
    with pytest.raises(ValidationError):
        G.model_validate({"i": -2, "f": 1.0})  # ge
    with pytest.raises(ValidationError):
        G.model_validate({"i": 10, "f": 1.0})  # lt
    with pytest.raises(ValidationError):
        G.model_validate({"i": 3, "f": 1.0})  # multiple_of
    with pytest.raises(ValidationError):
        G.model_validate({"i": 2, "f": 0.5})  # gt
    with pytest.raises(ValidationError):
        G.model_validate({"i": 2, "f": 1.6})  # le


def test_roundtrip_list_constraints_and_nested():
    G = _roundtrip(ListConstraints)
    ...
    with pytest.raises(ValidationError):
        G.model_validate({"items": [], "nested": [{"inner": "v", "tag": "a"}]})  # min_items
    with pytest.raises(ValidationError):
        G.model_validate({
